const app = getApp()
// pages/login/login.js
Page({
  data: {
    code: '',
    content: [],
    islogin: false,
    phone: "", // 手机号
    code: "", // 验证码
    agree: false, // 协议勾选
    isCountdown: false, // 倒计时状态
    countdownText: "获取验证码",
    countdown: 60 // 倒计时秒数
  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad(options) {
    if(decodeURIComponent(options.scene) == 'undefined'){
      wx.setStorageSync('code', "a1aec432-2ad3-11f0-af76-1ea122262551")
    }else{
      wx.setStorageSync('code', decodeURIComponent(options.scene))
    }

    let code = wx.getStorageSync('code')
    if (code) {
      console.log(code)
      let token = wx.getStorageSync('token')
      if (token) {
        this.get_list(token, code)
      }
    } else {
      app.fail_Remind('参数错误请联系商家')
      wx.navigateTo({
        url: "../index/index",
      })
    }
  },

  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady() {},

  /**
   * 生命周期函数--监听页面显示
   */
  onShow() {},

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide() {},

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload() {},

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function (e) {},

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom() {},

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage() {},

  // 协议勾选
  toggleAgree() {
    this.setData({
      agree: !this.data.agree
    });
  },

  // 手机号输入
  onPhoneInput(e) {
    this.setData({
      phone: e.detail.value
    });
  },

  // 验证码输入
  onCodeInput(e) {
    this.setData({
      code: e.detail.value
    });
  },

  // 发送验证码
  sendSmsCode() {

    const {
      phone,
      isCountdown
    } = this.data;

    // 1. 手机号验证
    if (!/^1[3-9]\d{9}$/.test(phone)) {
      wx.showToast({
        title: "手机号格式错误",
        icon: "none"
      });
      return;
    }

    // 2. 防止重复发送
    if (isCountdown) return;

    // 3. 启动倒计时
    this.setData({
      isCountdown: true
    });
    let timer = setInterval(() => {
      if (this.data.countdown <= 0) {
        clearInterval(timer);
        this.setData({
          isCountdown: false,
          countdownText: "重新发送",
          countdown: 60
        });
        return;
      }
      this.setData({
        countdownText: `${this.data.countdown}秒`,
        countdown: this.data.countdown - 1
      });
    }, 1000);

    // 4. 调用云函数发送短信
    wx.request({
      url: 'https://maneu.online/sendsms/',
      method: 'GET',
      data: {
        'code': this.data.phone,
      },
      success: (res) => {
        if (res.data.status == true) {
          app.fail_Remind('发送成功')
        } else {
          app.fail_Remind('获取验证码失败，请在次尝试')
        }
      },
      fail: (res) => {
        app.fail_Remind('网络异常，请在次尝试')
      }
    })
  },

  // 登录处理
  handleLogin() {
    const {
      phone,
      code,
      agree
    } = this.data;

    // 1. 协议校验
    if (!agree) {
      wx.showToast({
        title: "请阅读并同意协议",
        icon: "none"
      });
      return;
    }

    // 2. 验证码校验
    if (!/^\d{6}$/.test(code)) {
      wx.showToast({
        title: "请输入6位验证码",
        icon: "none"
      });
      return;
    }

    // 3. 登录请求
    let that = this
    wx.request({
      url: 'https://maneu.online/login/',
      method: "GET",
      data: {
        "call": that.data.phone,
        "code": that.data.code,
      },
      success(res) {
        if (res.data.status == true) {
          console.log(res.data)
          let code = wx.getStorageSync('code')
          wx.setStorageSync('token', res.data.token)
          that.get_list(res.data.token, code)
        } else {
          app.fail_Remind('登录失败，请在次尝试')
        }
      },
      fail: (res) => {
        app.fail_Remind('网络异常，请稍后再尝试')
      }
    });
  },

  // 微信快捷登录
  onWechatLogin(e) {
    const {
      agree
    } = this.data;

    // 1. 协议校验
    if (!agree) {
      wx.showToast({
        title: "请阅读并同意协议",
        icon: "none"
      });
      return;
    }
    if (e.detail.code) {
      let that = this
      // 通过code换取手机号（需后端解密）
      wx.request({
        url: 'https://maneu.online/login_wx/',
        method: "GET",
        data: {
          "code": e.detail.code
        },
        success(res) {
          console.log(res.data)
          if (res.data.status == true) {
            let code = wx.getStorageSync('code')
            wx.setStorageSync('token', res.data.token)
            that.get_list(res.data.token, code)
          } else {
            app.fail_Remind('登录失败，请在次尝试')
          }
        },
        fail: (res) => {
          app.fail_Remind('网络异常，请稍后再尝试')
        }
      });
    }
  },

  get_list(token, code) {
    wx.request({
      url: 'https://maneu.online/get_detail/',
      method: 'GET',
      data: {
        'token': token,
        'text': "100007",
        'code': code
      },
      success: (res) => {
        console.log(res.data)
        if (res.data.status) {
          this.setData({
            islogin: true,
            content: res.data.content
          })
          wx.setStorageSync('token', res.data.token)
        } else {
          wx.removeStorageSync('token')
        }
      },
      fail(res) {
        app.fail_remind("网络问题，请下拉刷新")
      }
    })
  },
})